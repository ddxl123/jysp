...
class MPnRulePoolNode implements MBase{
  ...
  MPnRulePoolNode.createModel({required int? aiid_v,required String? uuid_v,required RulePoolNodeType? type_v,required String? name_v,required String? position_v,required int? created_at_v,required int? updated_at_v,}) {
    _rowJson.addAll(<String, Object?>{aiid:aiid_v,uuid:uuid_v,type:type_v?.index,name:name_v,position:position_v,created_at:created_at_v,updated_at:updated_at_v,});
  }
  ...
  /// 若 [where]/[whereArgs] 为 null，则 query 的是全部 row。
  static Future<List<MPnRulePoolNode>> queryRowsAsModels({required String? where,required List<Object?>? whereArgs,required Transaction? connectTransaction}) async {
